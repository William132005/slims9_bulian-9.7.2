...
class Connection {
    ...
    public function buildConnectionArgument()
    {
        ...
        if ($this->driver === 'pdo') {
            ...
            // PERBAIKAN: Gunakan ?? untuk mengecek apakah variabel berasal dari 'username' atau 'user'
            $dsn[] = $username ?? $user;
            $dsn[] = $password;
            return $dsn;
        } 

        // PERBAIKAN: Gunakan ?? agar mendukung kedua format penamaan variabel
        return [$host, ($username ?? $user), $password, ($database ?? $name), $port];
    }
